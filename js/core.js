...
function goPage(p, fromHistory){
  const changed = curPage!==p;
  curPage = p;
  ...
  // 기기 뒤로가기 지원: 탭 이동을 히스토리에 기록 (뒤로가기로 복귀한 경우는 제외)
  if(!fromHistory && changed) navPush({page:p});
}

/* =================================================================
   앱 내 히스토리 (기기 뒤로가기 버튼 대응)
   - 탭 이동 → 뒤로가기 시 직전 탭으로 복귀
   - 모달(레벨 가이드·인바디·스토리 미리보기·⌘K) → 뒤로가기 시 모달만 닫힘
   ================================================================= */
const MODALS={};   // name -> {isOpen, close}  (close는 DOM만 닫는 raw 함수)
function registerModal(name, isOpen, close){ MODALS[name]={isOpen,close}; }

function navPush(state){
  try{ history.pushState({app:1, page:curPage, ...state}, ''); }catch(e){}
}
// 모달 열 때 호출 — 뒤로가기 한 번에 이 모달이 닫히도록 기록
function navPushModal(name){ navPush({modal:name}); }
/* 모달을 닫기 버튼·Esc·배경 클릭으로 닫을 때 호출.
   현재 히스토리 항목이 그 모달이면 back()으로 되감아 히스토리를 깨끗하게 유지 */
function navCloseModal(name){
  const m=MODALS[name];
  if(m && m.isOpen()) m.close();          // 먼저 실제로 닫고
  const st=history.state;
  if(st && st.app && st.modal===name){    // 남은 히스토리 항목 정리
    try{ history.back(); }catch(e){}
  }
}
/* 모달 안에서 다른 화면으로 이동할 때(예: ⌘K 실행, 레벨 가이드에서 멤버 클릭).
   back() 대신 현재 모달 항목을 일반 항목으로 교체해 이동과 충돌하지 않게 함 */
function navReplaceModal(){
  const st=history.state;
  if(st && st.app && st.modal){
    try{ history.replaceState({app:1, page:curPage}, ''); }catch(e){}
  }
}
function navInit(){
  try{ history.replaceState({app:1, page:curPage}, ''); }catch(e){}
...
window.addEventListener('popstate', ev=>{
  const st=(ev.state && ev.state.app) ? ev.state : {page:curPage};
  // 1) 목표 상태에 없는 열린 모달은 모두 닫기
  Object.keys(MODALS).forEach(n=>{
    const m=MODALS[n];
    if(m.isOpen() && st.modal!==n) m.close();
  });
  // 2) 탭 복귀
  if(st.page && st.page!==curPage) goPage(st.page, true);
});
...
function parse(rows){
  ...
  if(!S.selMonth) S.selMonth=nowYM;   // 기록이 하나도 없어도 화면이 뜨도록
...
function chartFont(){const s=getComputedStyle(document.documentElement);return{
  ...
  pink:s.getPropertyValue('--pink').trim(),cyan:s.getPropertyValue('--cyan').trim(),
};}
