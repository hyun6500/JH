...
function openLevelGuide(){
  ...
  navPushModal('lvl');   // 뒤로가기 한 번에 닫히도록
}
function lvlRawClose(){document.getElementById('lvlOverlay').classList.remove('open');}
function closeLevelGuide(){navCloseModal('lvl');}
// 레벨 가이드에서 멤버 칩 클릭 → 모달 닫고 그 멤버 프로필로 이동
function levelGoMember(n){lvlRawClose();navReplaceModal();openMember(n);}
registerModal('lvl',
  ()=>document.getElementById('lvlOverlay').classList.contains('open'),
  lvlRawClose);
...
function cmdkRawClose(){document.getElementById('cmdkOverlay').classList.remove('open');}
function closeCmdk(){navCloseModal('cmdk');}
registerModal('cmdk',
  ()=>document.getElementById('cmdkOverlay').classList.contains('open'),
  cmdkRawClose);
function cmdkFilter(){
...
function cmdkRun(i){
  ...
  if(!a)return;
  cmdkRawClose();
  navReplaceModal();   // back() 대신 교체 — 이어지는 탭 이동과 충돌 방지
  a.run();
}
function cmdkKey(ev){
