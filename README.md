...
js/
  ...
  celebrate.js      ← 축하 소식 엔진 + 홈 롤링 배너 / 개인 탭 축하 카드
...
## 단일 HTML 병합본 (새 채팅에 파일 하나만 첨부할 때)
```
node build-single.js
```
→ `ounwan-single.html` 생성 (css + js 10개가 모두 인라인된 단일 파일).
AI에게 작업을 맡길 때 이 파일 하나만 첨부하면 전체 코드를 전달할 수 있습니다.
**배포에는 사용하지 마세요** — 실제 서비스는 분리 구조(index.html)를 유지하고,
병합본은 어디까지나 "전달용 스냅샷"입니다.
(수정은 항상 분리 파일에 하고, 필요할 때 다시 병합본을 생성하세요)


## 함께 들어 있는 문서
- `DOCS.md` — 웹앱·Apps Script·스프레드시트의 구조와 규칙 전체 설명
  (탭 구조, API 명세, 도메인 규칙, 운영 체크리스트, 장애 진단 순서)
- `NEXT-CHAT-PROMPT.md` — 새 채팅에서 작업을 이어갈 때 쓰는 프롬프트 템플릿
